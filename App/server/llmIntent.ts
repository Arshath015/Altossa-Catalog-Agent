...
function buildSystemPrompt(productNames: string[], lastProduct: string | null): string {
  ...
  return [
    ...
    '  If one name in the list CONTAINS every word the person used, and a DIFFERENT name ',
    '  only contains SOME of them, that is still a SINGLE product the person means, not ',
    '  two -- include ONLY the more complete name, not both. E.g. for "cuff pouf", include ',
    '  only "Cuff bench and pouf" (it covers both "cuff" and "pouf"); do not also include ',
    '  plain "Cuff" alongside it, since that would wrongly treat one product as two.',
