...
# Some products' own real accessory (a "cover - art. XXXX" replacement
# cover, or a dash-referenced add-on like Dolmen's own "additional
# extension ... - art. 1820A") prints its price on a DIFFERENT physical
# line from its own dash-prefixed "- art." reference -- the reference line
# itself carries no price at all, so the existing same-line dash/cover
# exclusion never sees it. Confirmed on Flexion p330 (codes 25170/25171)
# and p332 (code 25176) for the cover case, and on Dolmen p211/p212 (all 4
# codes: 1821/1821L/1822/1822L) for the "additional extension" case --
# each Dolmen page's own extension accessory (art. 1820A, EUR 638
# standard / EUR 803 premium, identical on both pages -- a real, shared,
# genuinely distinct product currently invisible to the catalog since it
# has never been given its own catalog_index.json entry, flagged
# separately rather than decided here) prints its 2-tier price 1-10 lines
# away from its own "- art. 1820A" mention.
#
# A general "off-column price near a dash-art reference" rule was tried
# and reverted: several OTHER Varaschini products (e.g. Emma's own
# dual-material Daybed/Divano/Dormeuse grids, p232) legitimately have TWO
# real price columns per tier (Aluminio/Legno structure choice), and a
# coincidental cover/dash-art mention nearby was enough to wrongly exclude
# 90 real rows in a full-catalog dry run before this was caught. Hand-
# verified, narrowly scoped to the exact (page, price) pairs confirmed
# real -- same precedent as extract_catalog.py's own
# VARASCHINI_FALSE_POSITIVE_CODES -- rather than any general column or
# proximity heuristic. Flexion's own 25176 shares its €308 cover price
# with sibling 25175 on the SAME page, but on 25175's own SAME line as
# "cover - art. 9476C" (already safely excluded above, before this check
# is ever reached for that occurrence).
_VARASCHINI_WRAPPED_ACCESSORY_PRICES: set[tuple[int, str]] = {
    (330, "132"),  # Flexion 25170, cover - art. 9442C
    (330, "193"),  # Flexion 25171, cover - art. 9426C
    (332, "308"),  # Flexion 25176, cover - art. 9476C
    (211, "638"),  # Dolmen 1821/1821L, additional extension - art. 1820A (standard)
    (211, "803"),  # Dolmen 1821/1821L, additional extension - art. 1820A (premium)
    (212, "638"),  # Dolmen 1822/1822L, additional extension - art. 1820A (standard)
    (212, "803"),  # Dolmen 1822/1822L, additional extension - art. 1820A (premium)
}
        ...
        for li, bl in enumerate(block_lines):
            ...
            for m in VARASCHINI_PRICE_RE.finditer(bl):
                ...
                if prefix.strip() == "9c5130":
                    continue
                if (page_num, m.group(1)) in _VARASCHINI_WRAPPED_ACCESSORY_PRICES:
                    continue
