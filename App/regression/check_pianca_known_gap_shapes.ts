...
const CASES: Case[] = [
  ...
  {
    id: 'alfa-letti-wildcard-code-wxh-size',
    query: 'Alfa Letti price for 90x190',
    productName: 'Alfa (Letti)',
    code: 'WAF * 03S',
    expectedPrice: '1.197',
    expectedSize: '90x190',
    expectedTier: 'Materico',
    note: 'Resolved 2026-08-31 -- the other originally-named "outlier" pair (with Grafica/Kyoto). Superficially matches the already-built Letti tier-ladder shape (bare "L P CODICI" header, "WxH" nominal size, wildcard code) but has 4 NAMED finish columns (Materico/Laccato Opaco/Essenza/Cuoio) instead of the "A B C H P Q" tier-letter ladder -- confirmed via image, correctly declined by parse_file_pianca_letti_tier rather than mis-captured. New dedicated `_pianca_alfa_row_scan`: the wildcard code ("WAF * 03S", asterisk as its own token) can\'t be matched by `_pianca_labels_before_codici_row_scan`\'s own code detector (requires the whole code as ONE token), needing a new 3-token wildcard consumer. `size` is deliberately the printed "WxH" nominal bed size (matching Letti tier\'s own established convention), not the internal L/P structural dims (93×203 here) that follow it.',
  },
  {
    id: 'alfa-letti-contenitore-2nd-subtable',
    query: 'Alfa Letti price for 140x190',
    productName: 'Alfa (Letti)',
    code: 'WAF * 12C',
    expectedPrice: '2.112',
    expectedSize: '140x190',
    expectedTier: 'Materico',
    note: 'Alfa (Letti)\'s own 2nd sub-table ("Letto con sommier" / "Con contenitore", page 10) -- same header/columns, different wildcard suffix ("...12C" vs the 1st table\'s "...12S"), confirms both tables resolve correctly under the same product.',
  },
  {
    id: 'alfa-tatami-shared-mechanism-own-prefix',
    query: 'Alfa Tatami price for 90x190',
    productName: 'Alfa (Tatami)',
    code: 'WZ3 * 03S',
    expectedPrice: '836',
    expectedSize: '90x190',
    expectedTier: 'Materico',
  ...
  {
    id: 'norma-cn-comodino-a-terra-3dim-size',
    query: 'Norma CollezioneNotte price for 41 42 45',
    productName: 'Norma (CollezioneNotte)',
    code: '2N2Y4',
    expectedPrice: '376',
    expectedSize: '41×42×45',
    expectedTier: 'Copertura Laccato Opaco/Essenza — Frontali L. Opaco/Essenza',
    expectedVariantContext: 'Comodino',
    note: 'Live-reported gap 2026-09-02: this product had only its "Basamento" table (16 rows) extracted; 2 entire real tables (Comodino, Comò) were completely unbuilt. Guards the basic a-terra 3-dim (L×H×P) shape, with L correctly backward-filled from the group\'s own last row.',
  },
  {
    id: 'norma-cn-comodino-con-basamento-h-omitted',
    query: 'Norma CollezioneNotte price for 41 45 con basamento',
    productName: 'Norma (CollezioneNotte)',
    code: '2N2Y4B',
    expectedPrice: '957',
    expectedSize: '41×45',
    expectedVariantContext: 'Comodino',
    note: 'Guards that "con basamento" rows are captured as 2-dim (L×P only) since H is genuinely never reprinted for them in the source (confirmed via image, not a text-extraction drop) -- this project\'s "capture only what\'s printed" rule, not an invented H.',
  },
  {
    id: 'norma-cn-como-basso-carries-forward-no-own-label',
    query: 'Norma CollezioneNotte price for 91 42 45',
    productName: 'Norma (CollezioneNotte)',
    code: '2N2Y9',
    expectedPrice: '552',
    expectedSize: '91×42×45',
    expectedVariantContext: 'Comò basso',
    note: 'Guards the carry-forward category rule: this group (L=91) has NO trailing text label of its own before the table ends -- confirmed via image its own diagram icon matches the "Comò basso" style of the group just above it (L=71), so it correctly inherits that label rather than getting None or a wrong one.',
  },
  {
    id: 'norma-cn-settimanale-3digit-h-not-glued-to-prefix',
    query: 'Norma CollezioneNotte price for 71 127 45 settimanale',
    productName: 'Norma (CollezioneNotte)',
    code: '2N2T7',
    expectedPrice: '1.301',
    expectedSize: '71×127×45',
    expectedVariantContext: 'Settimanale',
    note: 'Root-cause guard for a real bug self-caught before committing: Settimanale\'s own H value is 3 digits (127, vs 42/52/62/77 elsewhere), which the source prints with only 1 space after "a terra" instead of 2+ -- "a terra 127" glued into one token under the original 2+-space split, leaving H silently None (silently dropping a real dimension) and corrupting the group\'s own L via a coincidental token-shape ambiguity. Fixed via position-aware segmentation (classifying a bare digit by real column distance to the header\'s own L vs H position, not token shape) plus an explicit glued-prefix regex.',
  },
  {
    id: 'norma-cn-category-changes-mid-table-comodino-then-como',
    query: 'Norma CollezioneNotte price for 71 62 45',
    productName: 'Norma (CollezioneNotte)',
    code: '2N2X7',
    expectedPrice: '590',
    expectedSize: '71×62×45',
